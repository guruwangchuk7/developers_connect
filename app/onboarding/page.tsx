...
export default function OnboardingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Synchronizing user identity for UUID:', user.id)

      const finalRole = selectedRole === "Other" ? customRole : selectedRole
      
      // Perform the identity synchronization (UPSERT)
      ...
      if (error) {
        console.error('DATABASE POLICY VIOLATION:', error)
        throw error
      }
      
      router.push('/dashboard')
    } catch (error: any) {
      console.error('Identity Setup Failed:', error.message || error)
      setIsLoading(false)
    }
  }
