...
export function PostCreator({
  ...
  const getPreviewContent = () => {
    if (activeTab === "help") return `BLOCKER: ${guidedFields.blocker}\nSTACK: ${guidedFields.stack}\nCONTEXT: ${guidedFields.context}`
    if (activeTab === "teams") return `ROLE NEEDED: ${guidedFields.role}\nPROJECT: ${guidedFields.project}\nMISSION: ${guidedFields.mission}`
    if (activeTab === "projects") return `PROJECT: ${guidedFields.projectName}\nDESCRIPTION: ${guidedFields.description}\nLINK: ${guidedFields.link}`
    return ""
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h3 className="text-[13px] font-semibold uppercase tracking-widest text-primary/60">
            {isPreview ? "Fragment Synthesis Preview" : "Guided Synthesis"}
          </h3>
        </div>
        <button 
          onClick={() => setIsPreview(!isPreview)}
          className="text-[10px] font-black uppercase tracking-widest text-primary/60 hover:text-primary transition-all border border-primary/20 px-3 py-1 rounded-sm"
        >
          {isPreview ? "Edit Source" : "Preview Fragment"}
        </button>
      </div>

      {isPreview ? (
        <div className="p-8 md:p-12 bg-secondary/20 border border-border/20 rounded-sm italic font-inter text-[15px] leading-relaxed text-foreground/70 whitespace-pre-wrap">
          {getPreviewContent() || "No context provided for synchronization."}
        </div>
      ) : (
      ...
      <div className="pt-6 border-t border-border/20 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-wrap items-center gap-4 md:gap-8">
          ...
          <div className="flex items-center gap-6">
            <button className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/30 hover:text-primary transition-all">#NextJS14</button>
            <button className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/30 hover:text-primary transition-all">#TailwindCSS</button>
            <button className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/30 hover:text-primary transition-all">#Rust</button>
          </div>
        </div>
